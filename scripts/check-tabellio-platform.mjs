...
import { platformRemoteRepository, validatePlatformConfig } from "./lib/platform-config.mjs";
...
try {
  ...
  const remoteRepository = platformRemoteRepository(config);
  console.log(JSON.stringify({
    ok: true,
    status: "platform_ready",
    path,
    schemaVersion: config.schemaVersion,
    remoteRepository: remoteRepository.provider,
    publicSurface: remoteRepository.publicSurface,
  }, null, 2));
} catch (error) {
  process.exitCode = 1;
